...
void status_loop(ServerPtr server)
{
	...
	UDPEpollServer* udpServer = dynamic_cast<UDPEpollServer*>(server.get());
	...
	while (running)
	{
		...
		cout<<"[work poll] : "<<udpServer->workerPoolStatus()<<endl;

		cout<<"[duplex poll] : "<<udpServer->answerCallbackPoolStatus()<<endl;
...
int main(int argc, char* argv[])
{
	try{
		...
		ServerPtr server = UDPEpollServer::create();
