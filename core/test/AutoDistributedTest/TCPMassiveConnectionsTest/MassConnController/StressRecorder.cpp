...
const std::vector<std::string> ArchivedRecordsFields{"Record time (UTC)", "Stress Connection Count",
	...
	"Interface QPS", "Server Connection Count", "load", "load/cpu", "ping/2 (msec)", "Recv Bytes/s", "Send Bytes/s"
};
...

	{
		...
		for (auto& pp: _archivedRecords)
		{
			for (auto& pp2: pp.second)
			{
				...
				//-- "Server Connection Count", "load", "load/cpu", "ping/2 (msec)", "Recv Bytes/s", "Send Bytes/s"
				ArchivedMachineStatus& ams = _archivedStatus[pp.first];
				row.push_back(ams.connCount.str());
...
bool StressRecorder::saveToCSVFormat(const std::string& filename)
{
	...
	ss << "Interface QPS, Server Connection Count, load, load/cpu, ping/2 (msec), Recv Bytes/s, Send Bytes/s";

	{
		...
		for (auto& pp: _archivedRecords)
		{
			for (auto& pp2: pp.second)
			{
				...
				//-- "Server Connection Count, load, load/cpu, ping/2 (msec), Recv Bytes/s, Send Bytes/s"
				ArchivedMachineStatus& ams = _archivedStatus[pp.first];
				ss << ams.connCount.str() << "," << ams.load.str() << ",";
