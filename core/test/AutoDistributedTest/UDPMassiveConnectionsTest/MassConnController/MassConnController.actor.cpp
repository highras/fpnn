...
bool StressController::gathermachineStatus(const std::set<std::string>& allDeployerEndpoints, std::vector<struct LoadStatus>& deployers)
{
	...
	int connIdx = findIndex("udpCount", fields);
	int cpusIdx = findIndex("cpus", fields);

	if (sourceIdx < 0 || hostIdx < 0 || loadIdx < 0 || memIdx < 0 || connIdx < 0 || cpusIdx < 0)
	{
		cout<<"[Exception] Machines status columns error. Cannot find 'source' or 'host' or 'load' or 'freeMemories' or 'udpCount'."<<endl;
		return false;
	}
...
void StressController::realSystemCmd(FPQuestPtr quest)
{
	FPAnswerPtr answer = _client->sendQuest(quest);
	FPAReader ar(answer);

	if (ar.status())
		cout<<"[System Command][Exception] Error code: "<<ar.wantInt("code")<<", ex: "<<ar.wantString("ex")<<endl;
	else
	{
		if (!ar.wantBool("ok"))
		{
			std::map<std::string, int> failedEndpoints = ar.want("failedEndpoints", std::map<std::string, int>());

			std::cout<<std::endl<<"System cmd failed at "<<failedEndpoints.size()<<" endpoint(s)."<<std::endl;
			std::cout<<"Failed endpoint(s):"<<std::endl;
			for (auto& pp: failedEndpoints)
				cout<<" "<<pp.first;
			cout<<endl;
		}
	}
...
void StressController::systemCmd(const std::set<std::string>& endpoints, const std::string& cmd)
{
	FPQWriter qw(2, "systemCmd");
	qw.param("endpoints", endpoints);
	qw.param("cmdLines", std::vector<std::string>{cmd});

	realSystemCmd(qw.take());
}
