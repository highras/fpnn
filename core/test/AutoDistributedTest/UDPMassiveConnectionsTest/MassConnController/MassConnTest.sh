...
function runTest()
{
	...
	$FakeAction sleep 2m
	$FakeAction ./UDPMassConnLoadWaiter $ControlCenter $TargetHost 0.02 7

	echo "Test $testIdx: Begin " `date` "Params: param: $1 thread, ${clientCount}w clients, per client QPS $3"  >> $LOG_FILE

	$FakeAction ./UDPMassConnController --contorlCenterEndpoint $ControlCenter --testEndpoint $TargetServer --stopPerCPULoad 2.1 --stopTimeCostMsec 800 --massThreadCount $1 --clientCount $2 --perClientQPS $3 --output $CSV_FILE --extraWaitTime $4 --actor ../MassConnActor/FPNNUDPMassiveConnectionsActor
