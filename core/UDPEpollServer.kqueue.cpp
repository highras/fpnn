...
bool UDPEpollServer::initIPv4()
{
	...
	struct kevent ev;
	EV_SET(&ev, _socket, EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, NULL);

	if (kevent(_kqueue_fd, &ev, 1, NULL, 0, NULL) == -1)
	{
		initFailClean("Add UDP IPv4 listening socket to kqueue failed.");
		return false;
	}
...
bool UDPEpollServer::initIPv6()
{
	...
	//-- Add Listening socket to kqueue
	struct kevent ev;
	EV_SET(&ev, _socket6, EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, NULL);

	if (kevent(_kqueue_fd, &ev, 1, NULL, 0, NULL) == -1)
	{
		initFailClean("Add UDP IPv6 listening socket to kqueue failed.");
		return false;
	}
...
bool UDPEpollServer::initKqueue()
{
	_kqueue_fd = kqueue();
	if (_kqueue_fd == -1)
	{
	...
	{
		initFailClean("Create kqueue events matrix for UDP server failed.");
		return false;
	}
...
void UDPEpollServer::run()
{
	...
	while (true)
	{
		...
		{
			if (_kqueueEvents[i].ident == _socket)
			{
				createConnections();
			}
			else if (_kqueueEvents[i].ident == _socket6)
			{
				createConnections6();
			}
			else
...
void UDPEpollServer::newConnection(int newSocket, bool isIPv4)
{
	...
	connection->kqueuefd = _kqueue_fd;
...
void UDPEpollServer::processEvent(struct kevent & event)
{
	...
	UDPServerConnection* conn = (UDPServerConnection*)_connectionMap.signConnection(socket, event.filter);
	if (conn)
		_ioPool->wakeUp(conn);
}
...
{
	struct kevent ev;
	EV_SET(&ev, socket, EVFILT_READ, EV_ADD | EV_CLEAR | EV_ONESHOT, 0, 0, NULL);
...
{
	struct kevent ev;
	EV_SET(&ev, socket, EVFILT_READ | EVFILT_WRITE, EV_ADD | EV_CLEAR | EV_ONESHOT, 0, 0, NULL);
...
{
	struct kevent ev;
	EV_SET(&ev, socket, EVFILT_READ | EVFILT_WRITE, EV_DELETE, 0, 0, NULL);
