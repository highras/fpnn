/*
 * The hash function here is from
...
#define hashbig		jenkins_hashbig
#define hashword	jenkins_hashword
#define hashword2	jenkins_hashword2


inline uint32_t jenkins_hash(const void *key, size_t length, uint32_t initval)
{
	return jenkins_hashlittle(key, length, initval);
}

inline void jenkins_hash2(const void *key, size_t length, uint32_t *pc, uint32_t *pb)
{
	jenkins_hashlittle2(key, length, pc, pb);
}

inline uint64_t jenkins_hash64(const void *key, size_t length, uint64_t initval)
{
	uint32_t c = initval;
	uint32_t b = initval >> 32;
	jenkins_hashlittle2(key, length, &c, &b);
