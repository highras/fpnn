...
#if defined(__GNUC__)
#define ATTRS_UNUSED		__attribute__((unused))
#else
#define ATTRS_UNUSED
#endif

#define CONCATENATE_DIRECT__(s1, s2) 	s1##s2
#define CONCATENATE__(s1, s2) 		CONCATENATE_DIRECT__(s1, s2)
#define ANONYMOUS_VARIABLE__(str) 	CONCATENATE__(str, __LINE__)

#define ON_BLOCK_EXIT 		ScopeGuard ANONYMOUS_VARIABLE__(_scopeGuard__) ATTRS_UNUSED = MakeGuard
#define ON_BLOCK_EXIT_OBJ 	ScopeGuard ANONYMOUS_VARIABLE__(_scopeGuard__) ATTRS_UNUSED = MakeObjGuard
