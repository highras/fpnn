...
class ParamTemplateThreadPool
{
	...
	private:
		...
		std::queue<K>			_taskQueue;
		std::queue<K>			_priorQueue;
...
bool ParamTemplateThreadPool<K>::wakeUp(K task)
{
	...
	if (_maxQueueLength && _maxQueueLength <= (int32_t)(_taskQueue.size() + _priorQueue.size()))
		return false;

	_taskQueue.push(task);

	if (_busyThreadCount + (int32_t)(_taskQueue.size() + _priorQueue.size()) > (_normalThreadCount + _tempThreadCount))
		append();		
...
void ParamTemplateThreadPool<K>::process()
{
	while (true)
	{
		...
		{
			std::unique_lock<std::mutex> lck(_mutex);
			while (_taskQueue.size() == 0 && _priorQueue.size() == 0)
			{
				if (_willExit)
				{
					_normalThreadCount -= 1;
...
void ParamTemplateThreadPool<K>::status(int32_t &normalThreadCount, int32_t &temporaryThreadCount, int32_t &busyThreadCount, int32_t &taskQueueSize, int32_t& min, int32_t& max, int32_t& maxQueue)
{
	if (_inited)
	{
		...
		taskQueueSize = (int32_t)(_taskQueue.size() + _priorQueue.size());
		min = _initCount;
		max = _maxCount;
