...
int MultipleURLEngine::Executor::socketCallback(CURL *easy, curl_socket_t s, int what, void *userp, void *socketp)
{
	...
	{
		epoll_ctl(executor->_epoll_fd, EPOLL_CTL_DEL, s, &event);
	...
	if (event.events)
	{
		if (epoll_ctl(executor->_epoll_fd, EPOLL_CTL_ADD, s, &event) != 0)
		{
			if (errno == EEXIST)
			{
				if (epoll_ctl(executor->_epoll_fd, EPOLL_CTL_MOD, s, &event) != 0)
					LOG_ERROR("MultipleURLEngine::Executor::socketCallback() mod socket to epoll failed.");
			}
			else
				LOG_ERROR("MultipleURLEngine::Executor::socketCallback() adds socket to epoll failed.");
		}
		else
		{
...
bool MultipleURLEngine::Executor::initEpoll()
{
	...
	ev.data.fd = _eventNotifyFds[0];
	ev.events = EPOLLIN | EPOLLERR | EPOLLHUP | EPOLLET | EPOLLRDHUP;

	if (epoll_ctl(_epoll_fd, EPOLL_CTL_ADD, _eventNotifyFds[0], &ev) != 0)
	{
		stopEpoll();
		return false;
	}
...
void MultipleURLEngine::Executor::stopEpoll()
{
...
void MultipleURLEngine::Executor::execute_thread()
{
	...
	while (_running)
	{
		...
		int readyfdsCount = epoll_wait(_epoll_fd, _epollEvents, _max_events, _waitMilliseconds);
		if (!_running)
		{
			clean();
			return;
		...
		{
			if (_epollEvents[i].data.fd == _eventNotifyFds[0])
			...
			else
			{
				...
				curl_multi_socket_action(_multi, _epollEvents[i].data.fd, curlMask, &running_handles);
				needActive = false;
			}
		}
